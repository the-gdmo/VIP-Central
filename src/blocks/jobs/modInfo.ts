...
function modInfoTemplate(context: JobContext): string {
    return (
        `# VIP Central Mod Info for r/${context.subredditName}\n\n` +
        `***This page is automatically managed by [VIP Central](https://reddit.com/u/vipcentral). Any edits will be overwritten.***\n\n` +
        `---\n\n` +
        `## General Information\n\n` +
        ...
        `You can update the bot in [your subreddit's VIP Central settings](https://developers.reddit.com/r/${context.subredditName}/apps/${context.appSlug}).`
    );
}
...
export async function modInfoJob(
    ...
    const wikiPath = "vipcentral/modinfo";
