...
export async function modDigestJob(
    ...
    try {
        ...
        const rawNewConversation = settings[AppSetting.DigestNewMessageEachDay];

        const newConversation = parseBooleanSetting(rawNewConversation, false);
        ...
        logger.info("⚙️ Mod digest settings loaded", {
            ...
            newConversation,
            rawNewConversation,
            rawNewConversationType: typeof rawNewConversation,
        });
        ...
        const redisPrefix = "vipcentral:modDigest";
        ...
        logger.debug("📜 Loading VIP Central audit entries", {
            subreddit: subredditName,
            auditKey: "vipcentral:audit:entries",
        });

        const rawAuditEntries = await context.redis.hGetAll(
            "vipcentral:audit:entries"
        );
        ...
        for (const [auditId, rawEntry] of Object.entries(rawAuditEntries)) {
            try {
                ...
                logger.warn("⚠️ Ignoring malformed VIP Central audit entry", {
                    subreddit: subredditName,
                    auditId,
                    error:
        ...
        const [
            ...
            context.redis.zRange("vipcentral:leaderboard:xp", 0, 4, {
                by: "rank",
                reverse: true,
            }),
            context.redis.zRange("vipcentral:leaderboard:coins", 0, 4, {
                by: "rank",
                reverse: true,
            }),
            context.redis.zRange("vipcentral:leaderboard:rep", 0, 4, {
                by: "rank",
                reverse: true,
            }),
            context.redis.zRange("vipcentral:leaderboard:streak", 0, 4, {
                by: "rank",
                reverse: true,
            }),
            context.redis.hGetAll("vipcentral:vips:expiry"),
        ]);
        ...
        const recentActivityLines = auditEntries.slice(0, 10).map((entry) => {
            ...
            const actor = entry.actor ? `u/${entry.actor}` : "VIP Central";
        ...
        const subject = `VIP Central ${frequency} Summary — ` + `r/${subredditName}`;

        let body = `# 👑 VIP Central ${frequency} Summary\n\n`;
        ...
        if (actionLines.length) {
            ...
            body += `_No audited VIP Central actions were recorded during this period._\n\n`;
        }
        ...
        body += `## 📜 Recent VIP Central Actions\n\n`;
        ...
        body += `\n\n---\n\n`;
        body += `*Generated automatically by VIP Central.*`;
        ...
        const desiredConversationType = "notification";
        ...
        if (!conversationId) {
            ...
            logger.debug("🔔 Sending digest to Mod Discussions", {
                subreddit: subredditName,
            });

            conversationId = await modMail.createModDiscussionConversation({
