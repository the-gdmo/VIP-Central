...
export async function onCommentSubmit(
    ...
    if (isBotCommand) {
        logger.info("🤖 Valid VIP Central command detected", {
    ...
    if (!("previousBody" in event)) {
        try {
            ...
            logger.error("❌ Failed to record VIP Central comment activity", {
                user: user.username,
                error,
            });
        }
    ...
    if (pointReplyAwardCommand) {
        ...
        const rewardKey =
            `vipcentral:replyReward:point:${parentComment.id}:` +
            normalizedCommentAuthor;
        const legacyCoinRewardKey =
            `vipcentral:replyReward:coin:${parentComment.id}:` +
            normalizedCommentAuthor;
        const legacyVipPointRewardKey =
            `vipcentral:replyReward:vip-point:${parentComment.id}:` +
            normalizedCommentAuthor;
